	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('jiajushangpin','新增')" type="primary" @click="add('/index/jiajushangpinAdd')">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/jiajufenlei/jiajufenlei').then(res => {
				if (res.data.code == 0) {
					this.jiajufenleiOptions = res.data.data;
					this.selectOptionsList.push({name:'家具分类',list:this.jiajufenleiOptions,tableName: 'jiajufenlei',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`jiajushangpin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiajushangpinDetail', query: params});
			},
