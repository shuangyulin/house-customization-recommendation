	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'——'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/cailiaofenlei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">材料分类</div>
						<div class="text "  >{{detail.cailiaofenlei}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('cailiaofenlei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('cailiaofenlei','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/cailiaofenlei', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/cailiaofenleiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此材料分类？') .then(_ => {
					this.$http.post('cailiaofenlei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
