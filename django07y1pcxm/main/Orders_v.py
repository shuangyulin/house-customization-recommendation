...
def orders_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and 'orders' != 'forum' :
            if req_dict.get("userid") and 'orders' != 'chat' and 'orders' != 'examrecord':
                del req_dict["userid"]
        else:
...
def orders_query(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = orders.objects.filter(**request.session.get("req_dict")).values()
...
def orders_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= orders.createbyreq(orders,orders, req_dict)
...
def orders_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM orders where ='{0}'".format(req_dict['username'])
        record = {}
...
def orders_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        where = ' where 1 = 1 '
        where = where +" AND status IN ('已支付', '已发货', '已完成') ";
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')
        ...
        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
...
def orders_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM orders {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def orders_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '季':
                sql = "SELECT CONCAT(YEAR(MIN({0})), '-Q', QUARTER(MIN({0}))) {0}, sum({1}) total FROM orders {2} GROUP BY YEAR({0}), QUARTER({0}) LIMIT 10".format(xColumnName, item, where)

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM orders {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
def orders_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        ...
        where = ' where 1 = 1 '
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM orders {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
